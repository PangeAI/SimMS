...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        refs = gr.File(
            label="Upload REFERENCES.mgf",
            interactive=True,
            value="tests/data/pesticides.mgf",
        )
        ques = gr.File(
            label="Upload QUERIES.mgf",
            interactive=True,
            value="tests/data/pesticides.mgf",
        )
    with gr.Row():
        ...
        intensity_power = gr.Slider(
            minimum=0, maximum=2, value=1.0, label="Intensity Power"
        )
    ...
    with gr.Row():
        batch_size = gr.Number(
            value=2048,
            label="Batch Size",
            info="How many spectra to process pairwise, in one step. Limited by GPU size, default works well for the T4 GPU.",
        )
        n_max_peaks = gr.Number(
            value=1024,
            label="Maximum Number of Peaks",
            info="Some spectra are too large to fit on GPU,"
            "so we have to trim them to only use the first "
            "n_max_peaks number of peaks.",
        )
        match_limit = gr.Number(
            value=2048,
            label="Match Limit",
    ...
    with gr.Row():
        out_npz = gr.File(
            label="Download similarity matrix (.npz format)", interactive=False
        )
        out_pickle = gr.File(
            label="Download full `Scores` object (.pickle format)", interactive=False
        )
    btn = gr.Button("Run")
    btn.click(
        fn=run,
