    ...
    [
        ...
        [
            [
                ...
                [
                    np.array([100, 300, 301, 511], dtype="float"),
                    np.array([0.1, 1.0, 0.3, 0.4], dtype="float"),
                ],
            ],
            0.2,
            0.0,
...
def test_cosine_greedy_pair(
    ...
    cosine_greedy = CudaCosineGreedy(
        batch_size=8,
        tolerance=tolerance,
        mz_power=mz_power,
    ...
    expected_score = compute_expected_score(
        mz_power, intensity_power, spectrum_1, spectrum_2, expected_matches
    )
...
def test_cosine_greedy_matrix(symmetric):
    ...
    assert scores[0][0][0] == pytest.approx(
        scores[1][1][0], 0.000001
    ), "Expected different cosine score."
    assert scores[0][0]["score"] == pytest.approx(
        scores[1][1]["score"], 0.000001
    ), "Expected different cosine score."
    assert scores[0][1][0] == pytest.approx(
        scores[1][0][0], 0.000001
    ), "Expected different cosine score."
    assert scores[0][1]["score"] == pytest.approx(
        scores[1][0]["score"], 0.000001
    ), "Expected different cosine score."
